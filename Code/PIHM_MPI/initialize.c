	...
	if (CS->init_type == 0)
  		{
			...
			NV_Ith_S(CV_Y, i + 4*DS->NumEle+DS->NumRiv)=(DS->Ele[i+DS->NumEle].zmax - DS->Ele[i+DS->NumEle].zmin) -0.1;											//xchen_20150406
  	...
  	/* data initialization mode */  
		...
		locI=0;									//xchen_20150329
				...
				if(DS->Ele[i].procNo-1 == my_pe)					//xchen_20150329_start
        				{   
      					DS->EleIS[locI] = DS->Ele_IC[i].interception;
                                	DS->EleSnow[locI]=DS->Ele_IC[i].snow;
                                	/* Note Two components can be separately read too */
                                	DS->EleSnowGrnd[locI]=(1-DS->Ele[i].VegFrac)* DS->EleSnow[i];
                                	DS->EleSnowCanopy[locI]=DS->Ele[i].VegFrac* DS->EleSnow[i];

...
void calcDirRiv(void *mData){
    ...
    MD = (Model_Data) mData;
	printf("\ncalcDirRiv...");
	printf("\nLine 166 Ele 1%d",MD->Ele[1].eleFlowDir[0]+MD->Ele[1].eleFlowDir[2]+MD->Ele[1].eleFlowDir[2]); 
	printf("\nLine 166 Ele 0%d",MD->Ele[0].eleFlowDir[0]+MD->Ele[0].eleFlowDir[2]+MD->Ele[0].eleFlowDir[2]);
	for(i=0; i<MD->NumRiv; i++){
		for(j=0; j<3; j++){
	...
	printf("Done!\n");
	printf("\nLine 198 Ele 1%d",MD->Ele[1].eleFlowDir[0]+MD->Ele[1].eleFlowDir[2]+MD->Ele[1].eleFlowDir[2]); 
	printf("\nLine 198 Ele 0%d",MD->Ele[0].eleFlowDir[0]+MD->Ele[0].eleFlowDir[2]+MD->Ele[0].eleFlowDir[2]);
}
...
void initialize(char *filename, Model_Data DS, Control_Data *CS, N_Vector CV_Y)
	{
  	int locI,m;								//xchen_20150329_start
	/* Store values from .init file*/
	realtype *y_init;							//xchen_20150411					  
	realtype *y_data;							//xchen_20150329
	/* Set pointer to data array CV_Y. */
    	y_data = NV_DATA_P(CV_Y);						//xchen_20150329_end
...
/*        if (CS->Debug == 1) 
		{
            	...
            	for(i=0; i<MD->NumRiv; i++)
			{
                	if(MD->Riv[i].LeftEle > 0) 
				{
				if(MD->Ele[MD->Riv[i].LeftEle-1].SINK == 1)
                    			{
					printf(" #L %d ", i+1);
					}
				}
                	if(MD->Riv[i].RightEle > 0) 
				{
				if(MD->Ele[MD->Riv[i].RightEle-1].SINK == 1)
					{                    
					printf(" #R %d ", i+1);
            				}
				}
			...
			printf("\n\n\nTrying to Fix Sinks Sharing Edge with Stream Reach...\n"); //getchar();
                	for(i=0; i<MD->NumRiv; i++)
				{
                    		if(MD->Riv[i].LeftEle > 0) if(MD->Ele[MD->Riv[i].LeftEle-1].SINK == 1)
					{
						...
						{ //this is necessary to make sure that the element is sink with respect to this particular stream segment
                            //printf(" L %d ", i+1);
	...
	if (CS->init_type == 0)
		...
		locI = 0;							//xchen_20150411_start
			...
			DS->EleSnowCanopy[i]=DS->Ele[i].VegFrac* DS->EleSnow[i];
      			y_data[locI] = 0;
	#ifdef SUB_SURF_RIV
      			y_data[locI+NumEleInProc[my_pe]] = 0;
			y_data[locI+2*NumEleInProc[my_pe]] = 0;
		#ifdef LAYER3
			y_data[locI+3*NumEleInProc[my_pe]] = 0;
		#endif
	#endif
			locI++;
			}
    		...
    		for(m=0; m<RivInProcCounter[my_pe]; m++)
				...
				y_data[locI + 3*NumEleInProc[my_pe]] = 0;
                        /* Note once the element beneath river is incorporated in decomposition and .mesh file, initialization should be perfomed based on the location data instead of average of neighbor properties */
                                y_data[locI + 3*NumEleInProc[my_pe] + NumRivInProc[my_pe]]=(DS->Ele[i+DS->NumEle].zmax - DS->Ele[i+DS->NumEle].zmin) -0.1;
			#endif 
      			#ifdef LAYER3
				y_data[locI + 4*NumEleInProc[my_pe]] = 0;
			/* Note once the element beneath river is incorporated in decomposition and .mesh file, initialization should be perfomed based on the location data instead of average of neighbor properties */
				y_data[locI + 4*NumEleInProc[my_pe] + NumRivInProc[my_pe]]=(DS->Ele[i+DS->NumEle].zmax - DS->Ele[i+DS->NumEle].zmin) -0.1;
    			#endif
	#endif	
				}
			locI++;
		...
		locI=0;									//xchen_20150329
			...
			i = EleInProc[my_pe][m]-1;
		        DS->EleIS[i] = DS->Ele_IC[i].interception;
                        ...
                        DS->EleSnowGrnd[i]=(1-DS->Ele[i].VegFrac)* DS->EleSnow[i];
                        DS->EleSnowCanopy[i]=DS->Ele[i].VegFrac* DS->EleSnow[i];
			if(DS->Ele[i].procNo-1 == my_pe)					//xchen_20150329_start
        			{   
				y_data[locI] = DS->Ele_IC[i].surf;
		#ifdef SUB_SURF_RIV
				y_data[locI+NumEleInProc[my_pe]] = DS->Ele_IC[i].unsat+0.1;
      				y_data[locI+2*NumEleInProc[my_pe]] = DS->Ele_IC[i].sat;
      			#ifdef LAYER3		
      				y_data[locI+3*NumEleInProc[my_pe]] = (DS->Ele[i].zmax-DS->Ele[i].zmin-y_data[locI+NumEleInProc[my_pe]]-y_data[locI+2*NumEleInProc[my_pe]])>0?0.75*(DS->Ele[i].zmax-DS->Ele[i].zmin-y_data[locI+NumEleInProc[my_pe]]-y_data[locI+2*NumEleInProc[my_pe]]):0.1;
			#endif   
		#endif
       				locI++;
      				}								//xchen_20150329_end
			}
		locI=0;										//xchen_20150329_start
                for(m=0; m<RivInProcCounter[my_pe]; m++)
                	{
               	#ifdef SUB_SURF_RIV  
			i = RivInProc[my_pe][m]-1;
			if(DS->Riv[i].proc-1==my_pe)
        			{
			#ifdef LAYER2		
				y_data[locI+3*NumEleInProc[my_pe]] = DS->Riv_IC[DS->Riv[i].IC-1].value;
				/* Note once the element beneath river is incorporated in decomposition and .mesh file, initialization should be perfomed based on the location data instead of average of neighbor properties */
                               	y_data[locI+3*NumEleInProc[my_pe]+NumRivInProc[my_pe]] = (DS->Ele[i+DS->NumEle].zmax - DS->Ele[i+DS->NumEle].zmin)-0.1;
			#endif
			#ifdef LAYER3           
                                y_data[locI+4*NumEleInProc[my_pe]] = DS->Riv_IC[DS->Riv[i].IC-1].value;
                        	/* Note once the element beneath river is incorporated in decomposition and .mesh file, initialization should be p
erfomed based on the location data instead of average of neighbor properties */                                        
				y_data[locI+4*NumEleInProc[my_pe]+NumRivInProc[my_pe]] = (DS->Ele[i+DS->NumEle].zmax - DS->Ele[i+DS->NumEle].zmin)-0.1;
			#endif
				locI++;	
				}
  	...
  	/* hot start mode */
  	else				
  		{
			...
			if(my_pe==0)
    			...
    			{
			/* allocate memory storage to y_init */
			y_init = (realtype *)malloc((DS->totele+2*DS->NumRiv)*sizeof(realtype));	
      			for(i=0; i<DS->NumEle; i++)
      				{
				...
				y_init[i]=tempvalue1;
		#ifdef SUB_SURF_RIV
				y_init[i + DS->NumEle]=tempvalue2;
				y_init[i + 2*DS->NumEle]=tempvalue3;
			#ifdef LAYER3	
				y_init[i + 3*DS->NumEle]=tempvalue4;
			#endif
      				}  
      			for(i=0; i<DS->NumRiv; i++)
      				{
        			fscanf(init_file, "%lf %lf",&tempvalue1,&tempvalue2);
        			y_init[i + DS->totele] = tempvalue1;
        			y_init[i + DS->totele + DS->NumRiv] = tempvalue2;
		#endif
      				} 
    			/* Initialize local y */
			locI=0;                                                                 //xchen_20150329
                	for(m=0; m<EleInProcCounter[my_pe]; m++)                                    
                        	{
                        	i = EleInProc[my_pe][m]-1;
				if(DS->Ele[i].procNo-1 == my_pe) 
					{
					y_data[locI] = y_init[i]; 
                #ifdef SUB_SURF_RIV 
                                	y_data[locI+NumEleInProc[my_pe]] = y_init[i + DS->NumEle];
                                	y_data[locI+2*NumEleInProc[my_pe]] = y_init[i + 2*DS->NumEle];
                        #ifdef LAYER3           
                        		y_data[locI+3*NumEleInProc[my_pe]] = y_init[i + 3*DS->NumEle];
					//
					printf("\n");                                        
					printf("Ele Init_Cond %d %d %lf %lf %lf %lf\n",m,i,y_data[locI],y_data[locI+NumEleInProc[my_pe]],y_data[locI+2*NumEleInProc[my_pe]],y_data[locI+3*NumEleInProc[my_pe]]);
					fflush(stdout);
			#endif  
                #endif
                                	locI++;
			...
			locI=0;                                                                         //xchen_20150329_start
                	for(m=0; m<RivInProcCounter[my_pe]; m++)
				{
                        	...
                        	if(DS->Riv[i].proc-1==my_pe)
                                	{
                        #ifdef LAYER2           
                                	y_data[locI+3*NumEleInProc[my_pe]] = y_init[i + DS->totele];
                                	y_data[locI+3*NumEleInProc[my_pe]+NumRivInProc[my_pe]] = y_init[i + DS->totele + DS->NumRiv];
                        #endif
                        #ifdef LAYER3           
                                	y_data[locI+4*NumEleInProc[my_pe]] = y_init[i + DS->totele];
                                	y_data[locI+4*NumEleInProc[my_pe]+NumRivInProc[my_pe]] = y_init[i + DS->totele + DS->NumRiv];
                       			//                                        
                       			printf("\n");                                        
                       			printf("Riv Init_Cond %d %d %lf %lf\n",m,i,y_data[locI+4*NumEleInProc[my_pe]],y_data[locI+4*NumEleInProc[my_pe]+NumRivInProc[my_pe]]);
         			       	fflush(stdout);
		 	#endif
                                	locI++;
                                	}

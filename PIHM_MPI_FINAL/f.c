...
//			printf("\n");
//                        printf("Ele States i=%d m=%d j=%d %lf\n",i,m,j,DummyY[i+j*MD->NumEle]);
//			fflush(stdout);
			}
	...
	locI=0;
		...
		if(MD->Riv[i].proc-1==my_pe)
			...
			DummyY[i+nly*MD->NumEle+MD->NumRiv]=(Y[locI+nly*NumEleInProc[my_pe]+NumRivInProc[my_pe]]>=0)?Y[locI+nly*NumEleInProc[my_pe]+NumRivInProc[my_pe]]:0;
			locI++;
			//
//			printf("\n");
//			printf("Riv States i=%d, m=%d, %lf, %lf\n",i,m,DummyY[i+nly*MD->NumEle],DummyY[i+nly*MD->NumEle+MD->NumRiv]);
//			fflush(stdout);
			}
//		else
//			{
...
//			}
	...
	printf("\n Just BEFORE communication %d",my_pe);                                         //xchen_20150402_end

        for(i=0;i<4*MD->NumEle+2*MD->NumRiv;i++)
                {   
//                printf("\nmy_pe = %d,i = %d,%lf",my_pe,i,DummyY[i]);
                } 
		...
		if(my_pe!=i)
			...
			if(my_pe>i)
				...
				for(j=0;j<RivEleSeqCounter[my_pe][i];j++)
					{
					comm_buff[j+2*RivSeqCounter[my_pe][i]+nly*EleSeqCounter[my_pe][i]]=DummyY[nly*MD->NumEle+RivEleNabrSequence[my_pe][i][j]];
					//xchen_20150423 RivEle return River index
					comm_buff[j+RivEleSeqCounter[my_pe][i]+2*RivSeqCounter[my_pe][i]+nly*EleSeqCounter[my_pe][i]]=DummyY[nly*MD->NumEle+MD->NumRiv+RivEleNabrSequence[my_pe][i][j]];					
					}
				for(j=0;j<EleRivSeqCounter[my_pe][i];j++)
                                        {
                                        comm_buff[j+2*RivEleSeqCounter[my_pe][i]+2*RivSeqCounter[my_pe][i]+nly*EleSeqCounter[my_pe][i]]=DummyY[EleRivNabrSequence[my_pe][i][j]];                                  
					//xchen_20150423 EleRiv return Element index
                                        comm_buff[j+EleRivSeqCounter[my_pe][i]+2*RivEleSeqCounter[my_pe][i]+2*RivSeqCounter[my_pe][i]+nly*EleSeqCounter[my_pe][i]]=DummyY[2*MD->NumEle+EleRivNabrSequence[my_pe][i][j]];             
                                        }
				if(EleSeqCounter[my_pe][i]+RivSeqCounter[my_pe][i]+RivEleSeqCounter[my_pe][i]+EleRivSeqCounter[my_pe][i]>0)
					...
					MPI_Send(&comm_buff[0],2*EleRivSeqCounter[my_pe][i]+2*RivEleSeqCounter[my_pe][i]+2*RivSeqCounter[my_pe][i]+nly*EleSeqCounter[my_pe][i], PVEC_REAL_MPI_TYPE,i,0, comm);
                     		//	printf("\nSent %d data from proc %d to proc %d",2*EleRivSeqCounter[my_pe][i]+2*RivEleSeqCounter[my_pe][i]+2*RivSeqCounter[my_pe][i]+nly*EleSeqCounter[my_pe][i],my_pe,i);
 	                       //        fflush(stdout);
					MPI_Recv(&comm_buff[0],2*EleRivSeqCounter[i][my_pe]+2*RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe],PVEC_REAL_MPI_TYPE,i,0,comm,&status);
                      		//	printf("\nReceived %d data from proc %d by proc %d",2*EleRivSeqCounter[i][my_pe]+2*RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe],i,my_pe);
                               //   	fflush(stdout);
					}
				...
				for(j=0;j<RivEleSeqCounter[i][my_pe];j++)
					{
					DummyY[RivEleNabrSequence[i][my_pe][j]+nly*MD->NumEle]=comm_buff[j+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]]<0?0:comm_buff[j+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]];
					DummyY[RivEleNabrSequence[i][my_pe][j]+nly*MD->NumEle+MD->NumRiv]=comm_buff[j+RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]]<0?0:comm_buff[j+RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]];		
					}
				for(j=0;j<EleRivSeqCounter[i][my_pe];j++)
                                        {   
                                        DummyY[EleRivNabrSequence[i][my_pe][j]]=comm_buff[j+2*RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]]<0?0:comm_buff[j+2*RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]];
					DummyY[2*MD->NumEle+EleRivNabrSequence[i][my_pe][j]]=comm_buff[j+EleRivSeqCounter[i][my_pe]+2*RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]]<0?0:comm_buff[j+EleRivSeqCounter[i][my_pe]+2*RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]];                       
                                        }	
				}
			else
				{
				if(EleSeqCounter[i][my_pe]+RivSeqCounter[i][my_pe]+RivEleSeqCounter[i][my_pe]+EleRivSeqCounter[i][my_pe]>0)
						...
						{
						DummyY[EleNabrSequence[i][my_pe][j]+k*MD->NumEle]=comm_buff[j+k*EleSeqCounter[i][my_pe]]<0?0:comm_buff[j+k*EleSeqCounter[i][my_pe]];                                
                        	//		printf("\nDummyY: k=%d, Ele Index=%d,%lf",k,EleNabrSequence[i][my_pe][j]+k*MD->NumEle,DummyY[EleNabrSequence[i][my_pe][j]+k*MD->NumEle]);
						}
					}
                        	for(j=0;j<RivSeqCounter[i][my_pe];j++)
                        		{    
                        	...
                        	for(j=0;j<RivEleSeqCounter[i][my_pe];j++)
                        		{    
                                	DummyY[nly*MD->NumEle+RivEleNabrSequence[i][my_pe][j]]=comm_buff[j+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]]<0?0:comm_buff[j+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]];					
					DummyY[nly*MD->NumEle+MD->NumRiv+RivEleNabrSequence[i][my_pe][j]]=comm_buff[j+RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]]<0?0:comm_buff[j+RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]];		
                        		}
				for(j=0;j<EleRivSeqCounter[i][my_pe];j++)
                                        {   
					DummyY[EleRivNabrSequence[i][my_pe][j]]=comm_buff[j+2*RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]]<0?0:comm_buff[j+2*RivEleSeqCounter[i][my_pe]+2*RivSeqCounter[i][my_pe]+nly*EleSeqCounter[i][my_pe]];
    		...
    		for(j=0; j<3; j++)
    			{
      			if(MD->Ele[i].nabr[j] > 0)
        			...
        			MD->FluxSub[i][j] = Avg_Ksat*Grad_Y*Avg_Y*MD->Ele[i].edge[j];
                       //         printf("\nFluxSub i=%d,j=%d,%lf",i,j,MD->FluxSub[i][j]);
			//	}
			#endif
		#ifndef NO_UNSAT
			    	/***************************************************************************/
                     ...
                     //   if(MD->Ele[i].Calc[j]>0)
                       ...
                       //  else
                        //	{
                    		...
                    		Avg_Sf=sqrt(pow(MD->Ele[i].dhBYdx,2)+pow(MD->Ele[i].dhBYdy,2));	//xchen_20150423_NEED ATTENTION
...
#endif
#endif
//		printf("After vertical flux calculation");
//		printf("\nEle DY i=%d,m=%d,%lf,%lf,%lf,%lf",i,m,DY[locI],DY[locI+NumEleInProc[my_pe]],DY[locI+2*NumEleInProc[my_pe]],DY[locI+3*NumEleInProc[my_pe]]);
  		locI++;
		}
		} 
/*****************************************************************************************************************/		//XCHEN_20150427
	/*****************Communicate lateral flux between elements*******************/
	for(i=0;i<MD->NumProc;i++)
		{
		if(my_pe!=i)
			{
			if(my_pe>i)
				...
				for(j=0;j<EleSeqCounter[my_pe][i];j++)
					...
					for(k=0;k<3;k++)
						{
						comm_buff[j*3+k]=MD->FluxSurf[EleNabrSequence[my_pe][i][j]][k];
					//	printf("\n Sending %d,%d,%lf",EleNabrSequence[my_pe][i][j],k,MD->FluxSurf[EleNabrSequence[my_pe][i][j]][k]);	
						}
					}
				if(EleSeqCounter[my_pe][i]>0)
					{
					MPI_Send(&comm_buff[0],3*EleSeqCounter[my_pe][i], PVEC_REAL_MPI_TYPE,i,0, comm);
					MPI_Recv(&comm_buff[0],3*EleSeqCounter[i][my_pe],PVEC_REAL_MPI_TYPE,i,0,comm,&status);
					}
				for(j=0;j<EleSeqCounter[i][my_pe];j++)
					{
					for(k=0;k<3;k++)
						{
						MD->FluxSurf[EleNabrSequence[i][my_pe][j]][k]=comm_buff[j*3+k];
					//	printf("\n Receiving by %d from %d,%d,%d,%lf",i,my_pe,EleNabrSequence[i][my_pe][j],k,MD->FluxSurf[EleNabrSequence[i][my_pe][j]][k]);
						}
					}
				}
			else
				{
				if(EleSeqCounter[i][my_pe]>0)
                        		{
                        		MPI_Recv(&comm_buff[0],3*EleSeqCounter[i][my_pe],PVEC_REAL_MPI_TYPE,i,0,comm,&status);
                        		}    
                        	for(j=0;j<EleSeqCounter[i][my_pe];j++)
						...
						{
                                                MD->FluxSurf[EleNabrSequence[i][my_pe][j]][k]=comm_buff[j*3+k];
                                       //         printf("\n Receiving %d,%d,%lf",EleNabrSequence[i][my_pe][j],k,MD->FluxSurf[EleNabrSequence[i][my_pe][j]][k]);
						}
					}
                        	for(j=0;j<EleSeqCounter[my_pe][i];j++)
                        		{    
...
/*****************************************************************************************************************/		//xchen_20150427
	for(m=0; m<EleInProcCounter[my_pe]; m++)
                {
                i=EleInProc[my_pe][m]-1;
		printf("\n Right after element lateral flux communication");
		for(j=0;j<3;j++)
			{
			printf("\nmy_pe=%d,i=%d,j=%d,FluxSurf=%lf",my_pe,i,j,MD->FluxSurf[i][j]);
			}
		}
                	...
                	for(i=0;i<MD->NumProc;i++)
                        	...
                        	if(my_pe!=i && EleRivSeqCounter[my_pe][i]>0)
                                	...
                                	for(l=0;l<EleRivSeqCounter[my_pe][i];l++)
                                        	{   
                                        	j=EleRivNabrSequence_RivIndex[my_pe][i][l];
	...
	for(m=0; m<RivInProcCounter[my_pe]; m++)
                {   
                i=RivInProc[my_pe][m]-1;
                printf("\nmy_pe=%d,i=%d,j=%d,FluxRiv[left]=%lf,FluxRiv[right]=%lf",my_pe,i,j,MD->FluxRiv[i][2],MD->FluxRiv[i][3]);
                } 
//for both river in this processor & element in this processor with neighboring river in other processor(s) xchen_20150405_end		
	...
	for(m=0; m<EleInProcCounter[my_pe]; m++)
		...
		#endif
		#endif
			printf("\nAfter lateral flux calculation");
			printf("\nEle DY i=%d \t my_pe=%d \t %lf \t %lf\t %lf \t %lf \n",i,my_pe,DY[locI],DY[locI+NumEleInProc[my_pe]],DY[locI+2*NumEleInProc[my_pe]],DY[locI+3*NumEleInProc[my_pe]]);
    			printf("\nEle Y i=%d \t my_pe=%d \t %lf \t %lf\t %lf \t %lf \n",i,my_pe,Y[locI],Y[locI+NumEleInProc[my_pe]],Y[locI+2*NumEleInProc[my_pe]],Y[locI+3*NumEleInProc[my_pe]]);
			locI++;
			}
		}
	...
	#endif
			printf("\nAfter lateral flux calculation");
			printf("\nRiv DY i=%d,my_pe=%d,%lf,%lf",i,my_pe,DY[locI+nly*NumEleInProc[my_pe]],DY[locI+nly*NumEleInProc[my_pe]+NumRivInProc[my_pe]]);
    			printf("\nRiv Y i=%d,my_pe=%d,%lf,%lf",i,my_pe,Y[locI+nly*NumEleInProc[my_pe]],Y[locI+nly*NumEleInProc[my_pe]+NumRivInProc[my_pe]]);
			locI++;
			}
		}
	free(DummyY);

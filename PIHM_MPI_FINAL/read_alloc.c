...
void read_alloc(char *filename, Model_Data DS, Control_Data *CS)				
	...
	fn[0] = (char *)malloc((strlen(filename)+5)*sizeof(char));
        ...
        fscanf(mesh_file,"%d %d %d %d", &DS->NumEle, &DS->NumRiv,&DS->NumNode,&DS->NumProc);
	...
	/* read in elements information */
        ...
        for(i=0;i<DS->NumProc;i++)
                ...
                for(j=0;j<DS->NumProc;j++)
                        {
                        EleNabrSequence[i][j]=(int *)malloc(NumEleInProc[i]*sizeof(int));		//should it be malloc(NumEleInProc[i]*3???	xchen_20150423)
                        EleSeqCounter[i][j]=0;
                        }
                }
	for (i=0; i<DS->NumEle; i++)
...
/*					printf("\n");
  ...
  */                                      }
	...
	EleRivNabrSequence_RivIndex=(int ***)malloc((DS->NumProc)*sizeof(int **));
	EleRivSeqCounter_RivIndex=(int **)malloc((DS->NumProc)*sizeof(int *));
	for(i=0;i<DS->NumProc;i++)
  		{
		...
		EleRivNabrSequence_RivIndex[i]=(int **)malloc((DS->NumProc)*sizeof(int *));
                EleRivSeqCounter_RivIndex[i]=(int *)malloc((DS->NumProc)*sizeof(int));
		for(j=0;j<DS->NumProc;j++)
        		{   
                	RivNabrSequence[i][j]=(int *)malloc(NumRivInProc[i]*sizeof(int));
                	RivSeqCounter[i][j]=0;
                	RivEleNabrSequence[i][j]=(int *)malloc(NumRivInProc[i]*sizeof(int));    /* Note the assumption here: Max(Number of elemental neighbors to river in each partition) =NumRivInProc[i] */ 			//xchen_20150404 2*NumRivInProc[i] rather than NumRivInProc[i] 
                	RivEleSeqCounter[i][j]=0;
        		EleRivNabrSequence[i][j]=(int *)malloc(NumEleInProc[i]*sizeof(int));    /* Note the assumption here: Max(Number of river neighbors to element in each partition) = 3*NumEleInProc[i] */                        //xchen_20150404 worst case scenario 
                        EleRivSeqCounter[i][j]=0;
			EleRivNabrSequence_RivIndex[i][j]=(int *)malloc(3*NumEleInProc[i]*sizeof(int *));
                	EleRivSeqCounter_RivIndex[i][j]=0;
			}   
  		}
  	for (i=0; i<DS->NumRiv; i++)
  		{
                        ...
                        if(DS->Riv[i].proc!=DS->Riv[DS->Riv[i].down-1].proc)
                        	{
                                ...
                                printf("\n");
				printf("Riv %d %d %d %d",i,DS->Riv[i].proc-1,DS->Riv[DS->Riv[i].down-1].proc-1,DS->Riv[i].index-1);
				fflush(stdout);
				}
                        }
		if(DS->Riv[i].proc-1!=DS->Ele[DS->Riv[i].LeftEle-1].procNo-1)
                	{
                	RivEleNabrSequence[DS->Riv[i].proc-1][DS->Ele[DS->Riv[i].LeftEle-1].procNo-1][RivEleSeqCounter[DS->Riv[i].proc-1][DS->Ele[DS->Riv[i].LeftEle-1].procNo-1]]=DS->Riv[i].index-1;		//xchen_20150404 return element id rather than river id	
                	RivEleSeqCounter[DS->Riv[i].proc-1][DS->Ele[DS->Riv[i].LeftEle-1].procNo-1]++;
			EleRivNabrSequence[DS->Ele[DS->Riv[i].LeftEle-1].procNo-1][DS->Riv[i].proc-1][EleRivSeqCounter[DS->Ele[DS->Riv[i].LeftEle-1].procNo-1][DS->Riv[i].proc-1]]=DS->Riv[i].LeftEle-1;                                        			//xchen_20150404 
                        EleRivSeqCounter[DS->Ele[DS->Riv[i].LeftEle-1].procNo-1][DS->Riv[i].proc-1]++;		//xchen_20150404
                	EleRivNabrSequence_RivIndex[DS->Ele[DS->Riv[i].LeftEle-1].procNo-1][DS->Riv[i].proc-1][EleRivSeqCounter_RivIndex[DS->Ele[DS->Riv[i].LeftEle-1].procNo-1][DS->Riv[i].proc-1]]=DS->Riv[i].index-1;                                                                //xchen_20150404 
                        EleRivSeqCounter_RivIndex[DS->Ele[DS->Riv[i].LeftEle-1].procNo-1][DS->Riv[i].proc-1]++;          //xchen_20150404
			RivInProc[DS->Ele[DS->Riv[i].LeftEle-1].procNo-1][RivInProcCounter[DS->Ele[DS->Riv[i].LeftEle-1].procNo-1]]=DS->Riv[i].index;
                	...
                	//
			printf("\n");                                
			printf("Riv %d %d %d %d",i,DS->Riv[i].proc-1,DS->Ele[DS->Riv[i].LeftEle-1].procNo-1,DS->Riv[i].LeftEle-1);
                        fflush(stdout);
			}
        	if(DS->Riv[i].proc-1!=DS->Ele[DS->Riv[i].RightEle-1].procNo-1)
                	{
                	RivEleNabrSequence[DS->Riv[i].proc-1][DS->Ele[DS->Riv[i].RightEle-1].procNo-1][RivEleSeqCounter[DS->Riv[i].proc-1][DS->Ele[DS->Riv[i].RightEle-1].procNo-1]]=DS->Riv[i].index-1;			//xchen_20150404 return element id rather than river id?
                	RivEleSeqCounter[DS->Riv[i].proc-1][DS->Ele[DS->Riv[i].RightEle-1].procNo-1]++;
                	EleRivNabrSequence[DS->Ele[DS->Riv[i].RightEle-1].procNo-1][DS->Riv[i].proc-1][EleRivSeqCounter[DS->Ele[DS->Riv[i].RightEle-1].procNo-1][DS->Riv[i].proc-1]]=DS->Riv[i].RightEle-1;                                                          //xchen_20150404 
                        EleRivSeqCounter[DS->Ele[DS->Riv[i].RightEle-1].procNo-1][DS->Riv[i].proc-1]++;          //xchen_20150404
			EleRivNabrSequence_RivIndex[DS->Ele[DS->Riv[i].RightEle-1].procNo-1][DS->Riv[i].proc-1][EleRivSeqCounter_RivIndex[DS->Ele[DS->Riv[i].RightEle-1].procNo-1][DS->Riv[i].proc-1]]=DS->Riv[i].index-1;                                                          //xchen_20150404 
                        EleRivSeqCounter_RivIndex[DS->Ele[DS->Riv[i].RightEle-1].procNo-1][DS->Riv[i].proc-1]++;          //xchen_20150404
			RivInProc[DS->Ele[DS->Riv[i].RightEle-1].procNo-1][RivInProcCounter[DS->Ele[DS->Riv[i].RightEle-1].procNo-1]]=DS->Riv[i].index;
                	...
                	//
			printf("\n");                                
                        printf("Riv %d %d %d %d",i,DS->Riv[i].proc-1,DS->Ele[DS->Riv[i].RightEle-1].procNo-1,DS->Riv[i].RightEle-1);
                        fflush(stdout);
			}
   		}												//xchen_20150328_end 
